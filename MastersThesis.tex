...
\begin{flalign*}
G_s(x) &= x^3 \\
&\hphantom{{}=x}- x^2 ( 2 a_1 + 2 a_2 + 2 a_3 + a_1 a_2 s + a_1 a_3 s + a_2 a_3 s ) &\\
&\hphantom{{}=x+x}+ x( a_1 a_2 a_3^2 s^2 + a_1 a_2^2 a_3 s^2 + a_1^2 a_2 a_3 s^2 + a_1 a_2^2 s  + a_1 a_3^2 s  + a_2 a_3^2 s  + a_1^2 a_2 s  + a_1^2 a_3 s  + a_2^2 a_3 s  \\
&\hphantom{{}=x+x+x+x} + 6 a_1 a_2 a_3 s  + a_1^2  + a_2^2  + a_3^2  + 3 a_1 a_2  + 3 a_1 a_3  + 3 a_2 a_3 ) &\\
&\hphantom{{}=x+x+x}-a_1^2 a_2^2 a_3^2 s^3   - 2 a_1 a_2^2 a_3^2 s^2 - 2 a_1^2 a_2 a_3^2 s^2 - 2 a_1^2 a_2^2 a_3 s^2  - a_1^2 a_2^2 s - a_1^2 a_3^2 s - a_2^2 a_3^2 s \\
&\hphantom{{}=x+x+x+x} - 3 a_1 a_2 a_3^2 s - 3 a_1 a_2^2 a_3 s - 3 a_1^2 a_2 a_3 s  - a_1 a_2^2 - a_1 a_3^2 - a_2 a_3^2 - a_1^2 a_2 - a_1^2 a_3 - a_2^2 a_3 - 2 a_1 a_2 a_3 
\end{flalign*}
...
\begin{flalign*}
G_s(x) &= x^3 \\
&\hphantom{{}=x}- x^2 ( 2 e_1 + e_2 s ) &\\
&\hphantom{{}=x+x}+ x( e_1 e_3 s^2 + e_1 e_2 s  + e_1^2 - 2e_2  + 3 e_2 ) &\\
&\hphantom{{}=x+x+x}-e_3^2 s^3   - 2 e_2 e_3 s^2  - (e_2^2 - 2e_1e_3) s  - 3 e_1 e_3 s  - e_1 e_2 + e_3
\end{flalign*}
...
We need the coefficients of $G_s(x)$ to be real in order to use our induction step.  Recall that the induction step states that polynomials of degree $2^{n-1} q'$ with real valued coefficients have at least one complex root.  In the example proof we saw a concrete version of $G_s(x)$ in this example the three terms were multiplied out and the resulting coefficents for each $x^i$ term were composed of symmetric polynomials in the roots.  

We have the same situation.  $G_s(x)$ is constructed so that the coefficients, additive and multiplciative combinations of the $x_1, x_2, \ldots x_d$ are symmetric.  Since the coefficients are symmetric then by the fundamental thereom of symmetric polynomials we can express the additive and multiplciative combinations of the $x_1, x_2, \ldots x_d$ as elementary symmetric polynomials, just as we did in the example proof.  \emph{Is this true?} Again the elementary symmetric polynomials relate to the original polynomial and are equivalent to the coefficients of $R(x)$.  The coefficients of $R(x)$ are real, therefore the elementary symmetic polynomials are real.  Likewise the additive and multiplciative combinations of the $x_1, x_2, \ldots x_d$ are also real.
...
add $S_n$ to itself, but we need to be clever here
...
A model to make multiplication understandable is that it is repeated addition, let's apply that model here:
$$2 S_n = d(d-1)$$
...
So the sum of this series is $\frac{d(d-1)}{2}$.  Recall that $d=2^n q$, so $\frac{d(d-1)}{2} = \frac{2^n q (d-1)}{2} = 2^{n-1}q(d-1)$, but $q(d-1)$ is an odd number so $2^{n-1}q(d-1) = 2^{n-1}q'$.  
