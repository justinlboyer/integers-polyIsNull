...
\]
 In fact, any polynomial $f(x)$ in $\mathbb{R}\left[x\right]$
...
The basics of field extension can be most easily understood in the world of clock (modular) arithmetic.  Consider an algebraic structure that consists of five elements, $\{0,1,2,3,4\}$. I will call this a 5-clock arithmetic, because it has five elements.  It is possible to add in this structure,
...
3+3+3+3+4=1
\]
so on and so forth.  The last equation in the series highlights repeated addition, I can use this model of multiplication (repeated addition) to understand how multiplication functions in 5-clock arithmetic.
\begin{align*}
3+3+3+3+4 & =1\\
4\cdot3+4 & =1\\
...
I have a multiplicative inverse for 2 it is 3. I.e., $2 \cdot 3=1$.  Likewise there is a multiplicative inverse for $1,3,4$ namely $1^{-1}=1$, $3^{-1}=2$, $4^{-1}=4$.  I will use the multiplicative inverse to understand division in this system.\\

What is $2\div4 =?$ in 5-clock arithmetic.  Rephrasing as a multiplication problem, $ 2 = ? \cdot 4$.  I know 4 has a multiplicative inverse namely itself, so I can multiply the previous equation by 4, 
\[
2 \cdot 4 = ? \cdot 4 \cdot 4
\]
and $ 2\cdot 4=3$, $4 \cdot 4 =1$  in this clock 5 arithmetic, therefore $3=?$.  So 
\[
2\div4=3
\]
 in this system and I can divide, because I have a multiplicative inverse.
...
What the Egyptians did was re-frame the division problem as multiplication, then read off the answer.  For example, $3\div3=?$ re-framed as multiplication becomes $3 = ? \cdot 3$.  Now I look at the times table and see that 4 times 3 gives me 3, therefore $?=4$ and $3 \div 3=4$.  Remember division is multiplication inverted.  In other words, each division problem is really just a multiplication problem in reverse.  Next I will show you a structure that does not have a multiplicative inverse.
...
Consider a structure whose set of numbers consists of four elements $\{0,1,2,3\}$, I will call this a 4-clock arithmetic.
...
(x-a_{1})(x-a_{2})(x-a_{3})
\]
and $a_{1},a_{2},a_{3}$ are elements of this field $\mathbb{J}$.  This new field $\mathbb{J}$ may look very different than my original field, for example $\sqrt{2}$ might actually have a solution in my new field. Lastly I can't say that $a_{1},a_{2},a_{3}$ are my three roots, because
they may not even be complex valued, let alone real-valued.

\subsection*{Overview}
The previous process should be reminiscent of prime factorization.  In both instances we are decomposing something into pieces, which cannot be decomposed anymore.  In regards to prime factorization we had prime numbers.  In regards to the splitting field we have irreducible terms.  The main difference is that in prime factorization all the possible prime numbers are available.  Whereas in the splitting field, I have to go looking for the pieces.  But there is a place where all the possible roots are available and that is where we are going!
...
\]
Look at the left hand side, notice how $a_1$ and $a_2$ could interchange positions and maintain an equivalent expression, likewise with $a_3$.  Polynomials with this property are called symmetric, meaning we can interchange "variables'' of the polynomial and obtain the same polynomial.  In the previous instance we were considering the polynomial's variables to be $a_1, a_2, a_3$.  Hence we could interchange them and obtain the exact same polynomial.

Now let's examine the right hand side of the previous equation:
\[
...
polynomials. Viéte identified that a symmetric polynomial can be written as a polynomial whose coefficients are elementary symmetric polynomials.  Viéte discovered the necessary formulas and since his formulas only utilize multiplication and addition, they apply in any of the fields we might be interested in.
...
First notice how $a_{1}^{2}+a_{2}^{2}+a_{3}^{2}$ is symmetric in $a_1, a_2, a_3$.  I can interchange any of the variables and I'll have an equivalent expression.
...
the coefficients of the original polynomial. Since the $r_{1},r_{2},r_{3}$ are
...
When I expand $G_{s}(x)$ I get: %$$ G_s(x) = x^2 - x(a_1 + 2a_2 + a_3 + s (a_1 a_2 + a_2 a_3))  + a_1 a_2 + a_1 a_3 + a_2 a_3 + a_2^2 + s(a_1 a_2^2 + 2a_1 a_2 a_3 + a_2^2 a_3) + s^2 a_1 a_2^2 a_3$$
...
up with an equivalent equation. Thus the polynomial viewed as a function of $a_1, a_2, a_3$ is symmetric and $G_{s}(x)$ is symmetric in the coefficients $a_{1},a_{2},a_{3}$.
Since $G_{s}(x)$ is symmetric then I can use Viéte's formulas and rewrite
...
valued.  Recall that the $e_{1},e_{2},e_{3}$
...
such that $R(x)$ decomposes into linear factors.} In math jargon
...
are real valued stems from Viéte's formulas, which explain how to rewrite the coefficients of any symmetric polynomial as a elementary symmetric polynomials. 
...
One of the statements of the Fundamental Theorem of Algebra is:

\fbox{\begin{minipage}[c]{30em}%
Every single variable polynomial of degree $n$ has exactly $n$ roots %
\end{minipage}}\\

I sated this is really a corollary of what I will be referring to as the Fundamental
Theorem of Algebra, which is:\\

\emph{Any polynomial with complex coefficients greater than degree
zero has at least one complex root}.\\

Let's prove the corollary using the Fundamental Theorem of Algebra.
...
we know by the Fundamental Theorem of Algebra that $f_{0}(x)$ has
at least one complex root, let's call it $r_{1}$. Then we can factor
$r_{1}$ out of $f_{0}(x)$ using long division. We know this step
is legitimate, because in both the field of reals and complex we can
...
The original polynomial $C(x)$ had
complex coefficients, which when multiplied by its conjugate $\bar{C(x)}$ produced a polynomial with real coefficients, we called this
polynomial $R(x)$. The goal was to show that this polynomial
...
but whose roots were the roots of $R(x)$. So when we found $G_{s}(x)$'s
complex root, we also found $R(x)$ complex root.  We then used this result to reduce the degree of the initial polynomial, and find the complex roots.  Using this result we were able to find $n$ roots for an $n^{\text{th}}$ degree polynomial.  In other words, we were able to decompose any polynomial into its constituent parts.  Just as any integer may be decomposed into its prime factors, polynomials may be completely decomposed into linear factors in the complex plane.  So the Fundamental Theorem of Arithmetic is to Integers, as the Fundamental Theorem of Algebra is to polynomials.
