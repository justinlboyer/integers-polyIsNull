...
\[
x^{3}-r_{1}x^{2}+r_{2}x-r_{3}=\left(x-s_{1}\right)\left(x^{2}+bx+c\right).
\]
...
In order to alleviate this problem the playing field is extended to
a set of numbers whose size (cardinality) is not composite, for example
we could extend it to using five numbers. The multiplication table
for this new field is below. 
...
Each column (except 0) contains a full set of the numbers in the field.
This makes division in this field unique because multiplication is
unique. Also now we can call this set a field, because it has a unique
multiplicative inverse. The set with four elements is referred to
as a ring.
...
Consider a field of five numbers $\{0,1,2,3\}$, it is possible to add these numbers,
for example, $1+1=2$, $1+2=3$, as well as $0+1=1$ this is the additive
...
field we map the number 5 to the number 0, this implies the number
6 would be mapped to 1. I.e., 
\[
2+4=1
\]
\[
...
Now I can utilize repeated addition to investigate multiplication
in this field. 
\begin{align*}
3+3+3+3+4 & =1\\
4\cdot3+4 & =1\\
(3+1)\cdot4 & =1\text{ Using distributive property}\\
4\cdot4 & =1
\end{align*}
If $4\cdot4=1$, then multiplying both sides by 4, we get 
\begin{align*}
4\cdot4\cdot4 & =4\\
1\cdot4 & =4
\end{align*}
...
Let's take a closer look at multiplication of twos.  Notice that no matter what I multiply 2 by there is a unique answer. 
\begin{align*}
2\cdot0=0\\
2\cdot1=2\\
2\cdot2=4\\
...
split any polynomial up (find the splitting field). I am extending
...
In other words, I find some field $\mathbb{F}$ that allows me to
split 
\[
x^{3}-r_{1}x^{2}+r_{2}x-r_{3}
\]
...
{\color{red}\emph{How do we know we will have 3 roots in C?} We
can't, but it doesn't matter, the proof doesn't rely on that, right?}\\

I can't say that $a_{1},a_{2},a_{3}$ are my three roots, because
I might not even have three. Further the numbers $a_{1},a_{2},a_{3}$
are in the field $\mathbb{F}$ and do not necessarily carry any current
meaning in $\mathbb{R}$. In other words the car is taken apart in
our friends garage. I need to make sense of these roots $a_{1},a_{2},a_{3}$.

I know that $\mathbb{F}$ is a field so all the properties of addition
and multiplication hold. So I can distribute and multiply, when I
do this I get: 
\[
